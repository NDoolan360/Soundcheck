<script lang="ts">
	...
	const highRes = currentImage(0);
	const lowRes = currentImage(2);
	let background: HTMLImageElement;
	const setBackground = () => {
		if (background) background.src = $highRes;
	};

	$: (ui("theme", $lowRes) as Promise<IBeerCssTheme>).then(setBackground);
</script>

<img class="responsive" src="./ambient.gif" alt="" bind:this={background} />
