...
<script lang="ts">
    ...
    const duration = <T extends number>(from: T, to: T): number => Math.abs(from - to) * 10;
    const circleAtts = {
        cx: '50%',
        cy: '50%',
...
<style>
    ...
    svg.indeterminate circle.indicator {
        animation:
            ...
            circular-expand-contract infinite calc(4s / 3) cubic-bezier(0.4, 0, 0.2, 1),
            circular-delayed-rotate infinite calc(16s / 3) cubic-bezier(0.4, 0, 0.2, 1);
