...
>
	<span
		class:icon={checked
			? $$slots['switch-checked-icon']
			: $$slots['switch-unchecked-icon']}
	>
...
<style>
	...
	button:disabled {
		...
		background-color: rgb(
			var(--sm3-scheme-color-surface-container-highest) / 12%
		);
	}
	...
	button.checked span {
		...
		background-color: var(
			--sm3-comp-switch-color-foreground-on,
			rgb(var(--sm3-scheme-color-on-primary))
		);
		color: var(
			--sm3-comp-switch-color-background-on,
			rgb(var(--sm3-scheme-color-primary))
