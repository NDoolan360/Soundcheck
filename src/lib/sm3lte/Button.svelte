<script lang="ts">
    ...
    if (FAB === true || (FAB === false && hasFABType) || (FAB != false && $$slots.default)) FAB = 'medium';
...
<style>
    ...
    button {
        ...
        background-color: var(--sm3-comp-button-color-background, rgb(var(--_background-color)));
        box-shadow: 0 0 var(--_elevation, var(--sm3-sys-elevation-level0)) rgb(var(--sm3-scheme-color-shadow));
    ...
    button::before {
        ...
        background-color: var(--sm3-comp-button-color-text, rgb(var(--_text-color)));
        content: '';
        inset: 0;
        opacity: 0;
    }
